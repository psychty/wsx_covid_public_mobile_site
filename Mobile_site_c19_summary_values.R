...
libraries(c("readxl", "readr", "plyr", "dplyr", "ggplot2", "png", "tidyverse", "reshape2", "scales", 'zoo', 'stats',"rgdal", 'rgeos', "tmaptools", 'sp', 'sf', 'maptools', 'leaflet', 'leaflet.extras', 'spdplyr', 'geojsonio', 'rmapshaper', 'jsonlite', 'grid', 'aweek', 'xml2', 'rvest', 'officer', 'flextable', 'viridis', 'epitools', 'ukcovid19'))
...
mye_total_raw <- read_csv('http://www.nomisweb.co.uk/api/v01/dataset/NM_2002_1.data.csv?geography=1816133633...1816133848,1820327937...1820328318,2092957697...2092957703,2013265921...2013265932&date=2019&gender=0&c_age=200,209&measures=20100&select=date_name,geography_name,geography_code,geography_type,c_age_name,obs_value') %>%  
  rename(Population = OBS_VALUE,
...
download.file(paste0('https://www.ons.gov.uk/file?uri=%2fpeoplepopulationandcommunity%2fhealthandsocialcare%2fcausesofdeath%2fdatasets%2fdeathregistrationsandoccurrencesbylocalauthorityandhealthboard%2f2021/lahbtables2021week28.xlsx'),  paste0(github_repo_dir, '/Source_files/ons_mortality.xlsx'), mode = 'wb')
...
wsx_18_plus_first_doses <- as.numeric(vac_info_df_wsx_18_plus$Dose_1)
wsx_18_plus_second_doses <- as.numeric(vac_info_df_wsx_18_plus$Dose_2)
...
vac_info_df_wsx_18_64 <- vaccine_df_ltla %>% 
  filter(Name == 'West Sussex') %>% 
  filter(Age_group == '18-64 years')

wsx_18_64_first_doses <- as.numeric(vac_info_df_wsx_18_64$Dose_1)
wsx_18_64_second_doses <- as.numeric(vac_info_df_wsx_18_64$Dose_2)
wsx_18_64_denominator <- as.numeric(vac_info_df_wsx_18_64$Denominator)
wsx_18_64_first_doses_proportion <- as.numeric(vac_info_df_wsx_18_64$Dose_1 / vac_info_df_wsx_18_64$Denominator)
wsx_18_64_second_doses_proportion <- as.numeric(vac_info_df_wsx_18_64$Dose_2 / vac_info_df_wsx_18_64$Denominator)
...
vac_info_df <- vaccine_df_ltla %>% 
  ...
  pivot_longer(cols = c('Received first and second doses','Received first dose only', 'Not vaccinated'),
               names_to = 'Status',
               values_to = 'People') %>% 
  mutate(Status = factor(Status, levels = rev(c('Received first and second doses','Received first dose only', 'Not vaccinated'))))
...
vac_info_df_x_18_64 <- vac_info_df %>% 
  filter(Name == 'West Sussex') %>% 
  filter(Age_group == '18-64 years') %>% 
  mutate(Status_label = factor(Status_label, levels = unique(Status_label)))
...
vac_uptake_18_plus_gg <- ggplot(vac_info_df_x, aes(x = 1.9, 
                                                   y = People,
                                                   fill = Status_label)) +
...
grid.text(paste0('18+ have received'),
          just = "centre",
          x = unit(0.1675, "npc"),
          y = unit(0.395, "npc"),
          gp = gpar(col = "#000000",
...
vac_uptake_18_64_gg <- ggplot(vac_info_df_x_18_64, aes(x = 1.9, 
                                                       y = People,
                                                       fill = Status_label)) +
  geom_bar(stat="identity") +
  xlim(.1, 2.5) +
  ...
  scale_fill_manual(values = c("#d81670", "#f5a0c7", "#dbdbdb"),
                    breaks = rev(levels(vac_info_df_x_18_64$Status_label)),
                    name = '') +
  scale_colour_manual(values= "#ffffff", guide = FALSE) +
  theme_bw()+
  theme(axis.ticks=element_blank(),
...
print(vac_uptake_18_64_gg, 
      vp = vplayout(21:28, 1:10))

grid.text(paste0(round(wsx_18_64_first_doses_proportion * 100, 0), '%'),
          just = "centre",
          x = unit(0.1725, "npc"),
          y = unit(0.23, "npc"),
          gp = gpar(col = "#000000",
...
grid.text(paste0('18-64 received'),
          just = "centre",
          x = unit(0.1675, "npc"),
          y = unit(0.195, "npc"),
          gp = gpar(col = "#000000",
...
grid.text(paste0('been a total of ', format(wsx_18_64_first_doses, big.mark = ',')),
          just = "left",
          x = unit(0.28, "npc"),
          y = unit(0.26, "npc"),
          gp = gpar(col = "#000000",
...
grid.text(paste0(format(wsx_18_64_second_doses, big.mark = ','),' received two doses'),
          just = "left",
          x = unit(0.305, "npc"),
          y = unit(0.18, "npc"),
          gp = gpar(col = "#000000",
...
grid.text(paste0(format(wsx_18_64_first_dose_only, big.mark = ','),' first dose only'),
          just = "left",
          x = unit(0.305, "npc"),
          y = unit(0.155, "npc"),
          gp = gpar(col = "#000000",
...
grid.text(paste0(format(wsx_18_64_unvaccinated, big.mark = ','),' not vaccinated'),
          just = "left",
          x = unit(0.305, "npc"),
          y = unit(0.13, "npc"),
          gp = gpar(col = "#000000",
...
# grid.roundrect(x = unit(0.54, "npc"),
#                y = unit(0.2, "npc"),
#                width = unit(0.42, "npc"),
#                height = unit(0.2, "npc"),
#                just = "left",
...
grid.text(paste0('In the South East region on ', format(se_hospital_df$Date, '%A '), ordinal(as.numeric(format(se_hospital_df$Date, '%d'))), format(se_hospital_df$Date, ' %B'), ', there were '),
          just = "left",
          x = unit(0.56, "npc"),
          y = unit(0.28, "npc"),
          gp = gpar(col = "#ffffff",
...
grid.text(paste0(format(se_hospital_df$Patients_occupying_beds, big.mark = ','), ' COVID-19 positive patients in hospital beds.'),
          just = "left",
          x = unit(0.56, "npc"),
          y = unit(0.26, "npc"),
          gp = gpar(col = "#ffffff",
